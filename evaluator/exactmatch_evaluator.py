...
class ExactMatchEvaluator(BaseEvaluator):
    def __del__(self):
        for epi, n in self.epi_to_num_correct_action.items():
            logging.info(f"{epi} has {n} correct actions")
    ...
    def eval_impl(
        ...
        # logging.info("=======", len(gr_actions), len(agent_predicted_actions))
        # logging.info(gr_actions[-1])
        # logging.info(agent_predicted_actions[-1])
        ...
        for i, gr_action in enumerate(gr_actions):
            ...
            try:
                ...
                logging.info(f"failed to extract ui positions from file: {vh_paths[i]}")
